...
#region Various fill functions
	...
	protected void FilledFill (BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols)
	{
		...
		if (mFillAmount < 1f)
		{
            ...
            if (mFillDirection == FillDirection.Cornor1 ||
                mFillDirection == FillDirection.Cornor3 ||
                mFillDirection == FillDirection.Cornor7 ||
                mFillDirection == FillDirection.Cornor9 ||
                mFillDirection == FillDirection.Cornor13 ||
                mFillDirection == FillDirection.Cornor37 ||
                mFillDirection == FillDirection.Cornor79 ||
                mFillDirection == FillDirection.Cornor91 ||
                mFillDirection == FillDirection.Cornor17 ||
                mFillDirection == FillDirection.Cornor39 ||
                mFillDirection == FillDirection.Cornor1379)
            {
                float fill = Mathf.Lerp(0.5f, 0f, fillAmount);
                for (int corner = 0; corner < 9; ++corner)
                {
                    float fx0, fx1, fy0, fy1;

                    if (corner == 0 || corner == 1 || corner == 2) { fx0 = 0f; fx1 = fill; }
                    else if (corner == 3 || corner == 4 || corner == 5) { fx0 = fill; fx1 = 1f - fill; }
                    else { fx0 = 1f - fill; fx1 = 1f; }

                    if (corner == 0 || corner == 3 || corner == 6) { fy0 = 0f; fy1 = fill; }
                    else if (corner == 1 || corner == 4 || corner == 7) { fy0 = fill; fy1 = 1f - fill; }
                    else { fy0 = 1f - fill; fy1 = 1f; }
                    ...
                    { 
                        case 0:
                            if (mFillDirection == FillDirection.Cornor1 ||
                                ...
                                mFillDirection == FillDirection.Cornor1379)
                            {
                                mTempPos[0].y = mTempPos[3].y;
                                mTempPos[0].x = mTempPos[3].x;
                                mTempUVs[0].y = mTempUVs[3].y;
                                mTempUVs[0].x = mTempUVs[3].x; 
                            }
                            break;
                        case 2:
                            if (mFillDirection == FillDirection.Cornor3 ||
                                mFillDirection == FillDirection.Cornor13 ||
                                mFillDirection == FillDirection.Cornor37 ||
                                mFillDirection == FillDirection.Cornor39 ||
                                mFillDirection == FillDirection.Cornor1379)
                            ...
                            break;
                        case 6:
                            if (mFillDirection == FillDirection.Cornor7 ||
                                mFillDirection == FillDirection.Cornor17 ||
                                mFillDirection == FillDirection.Cornor37 ||
                                mFillDirection == FillDirection.Cornor79 ||
                                mFillDirection == FillDirection.Cornor1379)
                            {
                                mTempPos[3].y = mTempPos[2].y;
                                mTempPos[3].x = mTempPos[2].x;
                                mTempUVs[3].y = mTempUVs[2].y;
                                mTempUVs[3].x = mTempUVs[2].x;
                            }
                            break;
                        case 8:
                            if (mFillDirection == FillDirection.Cornor9 ||
                                mFillDirection == FillDirection.Cornor91 ||
                                mFillDirection == FillDirection.Cornor39 ||
                                mFillDirection == FillDirection.Cornor79 ||
                                mFillDirection == FillDirection.Cornor1379)
                            {
                                mTempPos[2].y = mTempPos[1].y;
                                mTempPos[2].x = mTempPos[1].x;
                                mTempUVs[2].y = mTempUVs[1].y;
                                mTempUVs[2].x = mTempUVs[1].x;
                            }
                            break;
                        default:
                            break;
